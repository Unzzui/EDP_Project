...
def api_critical_projects():
    ...
    try:
        ...
        df_edp["monto_aprobado"] = pd.to_numeric(
            df_edp["monto_aprobado"], errors="coerce"
        )

        # Filtrar proyectos críticos (> 30 días de espera)
        df_criticos = df_edp[
            (df_edp["dias_espera"] >= 30)
        ...
        for (proyecto, cliente), grupo in df_criticos.groupby(["proyecto", "cliente"]):
            ...
            jefe_proyecto = (
                grupo["jefe_proyecto"].iloc[0]
                if not grupo["jefe_proyecto"].empty
                else "Sin asignar"
            )
            valor_total = (
                grupo["monto_aprobado"].sum() / 1_000_000
            ...
            for _, row in grupo.iterrows():
                ...
                estado = (
                    "critico"
                    if row["dias_espera"] > 30
                    else "Riesgo" if row["dias_espera"] > 15 else "Pendiente"
                )

                fecha_emisión = (
                    row["fecha_emision"].strftime("%d/%m/%Y")
                    if not pd.isnull(row["fecha_emision"])
                    else "N/A"
                )
                ...
                edps_relacionados.append(
                    {
                        ...
                        "date": fecha_emisión,
                        "amount": round(
                            row["monto_aprobado"] / 1_000_000, 2
                        ),  # Convertir a millones
        ...
        proyectos_criticos = sorted(
            proyectos_criticos, key=lambda x: x["value"], reverse=True
        )

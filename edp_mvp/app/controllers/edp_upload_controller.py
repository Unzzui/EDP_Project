...
# Blueprint
edp_upload_bp = Blueprint('edp_upload', __name__, url_prefix='/edp/upload')
...
def validate_edp_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if n_edp <= 0:
                ...
                if 'proyecto' in data and data['proyecto']:
                    ...
                    existing_edp = check_duplicate_edp(n_edp, data['proyecto'])
                    if existing_edp:
                        errors.append(f'❌ DUPLICADO: Ya existe un EDP #{n_edp} para el proyecto "{data["proyecto"]}". No se pueden crear EDPs con el mismo número en el mismo proyecto.')
                        print(f"🚫 Validación fallida: EDP duplicado encontrado")
                    else:
                        print(f"✅ Validación exitosa: EDP #{n_edp} es único para proyecto '{data['proyecto']}'")
        except (ValueError, TypeError):
    ...
    if 'fecha_emision' in data and data['fecha_emision']:
        try:
            datetime.strptime(str(data['fecha_emision']), '%Y-%m-%d')
        except (ValueError, TypeError):
    ...
    if 'monto_propuesto' in data and data['monto_propuesto']:
        try:
            monto = float(str(data['monto_propuesto']).replace(',', ''))
            if monto <= 0:
    ...
    if 'monto_aprobado' in data and data['monto_aprobado']:
        try:
            float(str(data['monto_aprobado']).replace(',', ''))
        except (ValueError, TypeError):
            ...
            try:
                ...
                if cache_service.redis_client:
                    try:
                        ...
                        from ..utils.gsheet import clear_all_cache
                        clear_all_cache()
...
def prepare_edp_data(form_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Limpiar y convertir montos a enteros
    monto_propuesto = form_data.get('monto_propuesto', 0)
    if isinstance(monto_propuesto, str):
        monto_propuesto = monto_propuesto.replace(',', '').replace('.0', '')
    # Verificar si es NaN
    if pd.isna(monto_propuesto):
        monto_propuesto = 0
    monto_propuesto = int(float(monto_propuesto)) if monto_propuesto else 0
    
    monto_aprobado = form_data.get('monto_aprobado')
    if monto_aprobado and not pd.isna(monto_aprobado):
        if isinstance(monto_aprobado, str):
            monto_aprobado = monto_aprobado.replace(',', '').replace('.0', '')
        try:
            monto_aprobado = int(float(monto_aprobado))
        except (ValueError, TypeError):
            monto_aprobado = None
    else:
        monto_aprobado = None
    
    # Generar mes automáticamente desde fecha_emision
    try:
        fecha_emision = datetime.strptime(str(form_data['fecha_emision']), '%Y-%m-%d')
        mes = fecha_emision.strftime('%Y-%m')
    except:
        mes = datetime.now().strftime('%Y-%m')
    
    # Si no hay fecha estimada de pago, calcular 30 días desde emisión
    fecha_estimada_pago = form_data.get('fecha_estimada_pago')
    if not fecha_estimada_pago:
        try:
            fecha_emision_dt = datetime.strptime(str(form_data['fecha_emision']), '%Y-%m-%d')
            fecha_estimada_pago = (fecha_emision_dt + timedelta(days=30)).strftime('%Y-%m-%d')
        except:
            fecha_estimada_pago = (datetime.now() + timedelta(days=30)).strftime('%Y-%m-%d')
    
    # Procesar n_edp con validación de NaN
    n_edp_value = form_data.get('n_edp')
    if pd.isna(n_edp_value):
        raise ValueError("El número EDP no puede estar vacío")
    
    edp_data = {
        # Campos obligatorios
        'n_edp': int(float(n_edp_value)),
        'proyecto': str(form_data['proyecto']).strip(),
        'cliente': str(form_data['cliente']).strip(),
        'jefe_proyecto': str(form_data['jefe_proyecto']).strip(),
        'fecha_emision': str(form_data['fecha_emision']),
        ...
        # Campos opcionales
        'gestor': str(form_data.get('gestor', '')).strip() if form_data.get('gestor') and str(form_data.get('gestor')).strip() != '' else None,
        'fecha_envio_cliente': str(form_data['fecha_envio_cliente']) if form_data.get('fecha_envio_cliente') and str(form_data.get('fecha_envio_cliente')).strip() != '' and str(form_data.get('fecha_envio_cliente')).strip().lower() != 'nan' else None,
        'monto_aprobado': monto_aprobado,
        'fecha_estimada_pago': fecha_estimada_pago,
        'observaciones': str(form_data.get('observaciones', '')).strip() if form_data.get('observaciones') and str(form_data.get('observaciones')).strip() != '' else None,
        
        # Campos de seguimiento
        'registrado_por': form_data.get('registrado_por', 'admin'),
        'fecha_registro': form_data.get('fecha_registro', datetime.now().isoformat())
    }

...
// Suprimir errores de media que no afectan la funcionalidad
...
// Suprimir promesas rechazadas relacionadas con media
window.addEventListener('unhandledrejection', function(e) {
    if (e.reason && e.reason.message && e.reason.message.includes('media resource')) {
        e.preventDefault();
        return false;
    }
});
		...
		Sortable.create(list, {
			...
			onAdd: function (evt) {
				...
				if (mostrarModal) {
					...
					const useInternalId = internalId && internalId !== '' && internalId !== 'null';
					const apiUrl = useInternalId 
						? `/dashboard/api/get-edp-by-id/${internalId}`
						: `/dashboard/api/get-edp/${edpId}`;
...
function openEdpModal(edpId, item = null) {
  ...
  // Determinar qué API usar - PRIORIZAR SIEMPRE ID interno para evitar conflictos
  let apiUrl = null;
  ...
  if (isNumericId) {
    ...
    if (useInternalId) {
      ...
      // Como último recurso, intentar con n_edp pero advertir del riesgo
  ...
  fetch(apiUrl)
    ...
    .then(data => {
      ...
      // Detectar advertencias sobre múltiples EDPs con el mismo n_edp
      if (data._warning) {
        console.warn(`⚠️ ADVERTENCIA DE UNICIDAD: ${data._warning}`);
        console.warn(`⚠️ IDs múltiples encontrados: ${data._multiple_ids}`);
        // Mostrar advertencia al usuario si no se está usando ID interno
        if (!useInternalId) {
          console.error(`🚨 PROBLEMA CRÍTICO: Se están usando APIs no únicas para EDP con múltiples instancias.`);
          showToast(`⚠️ ADVERTENCIA: EDP ${edpId} tiene múltiples registros. Datos pueden ser incorrectos.`, 'warning');
        }
      }
...
function obtenerYActualizarDatosEDP(edpId, item, estadoDestino) {
	...
	// Verificar si tenemos ID interno disponible - SIEMPRE debe usarse cuando esté disponible
	const internalId = item.getAttribute('data-internal-id');
	const useInternalId = internalId && internalId !== '' && internalId !== 'null';
	
	if (!useInternalId) {
		console.warn(`⚠️ EDP ${edpId} no tiene ID interno disponible. Esto puede causar problemas si hay múltiples EDPs con el mismo n_edp.`);
		console.warn(`📋 Saltando actualización silenciosa para evitar conflictos de datos.`);
		// No intentar actualizar si no hay ID único disponible
		return;
	}
	
	// SIEMPRE usar ID interno para garantizar unicidad
	const apiUrl = `/dashboard/api/edp-details-by-id/${internalId}`;
	
	console.log(`🔍 Usando API: ${apiUrl} (ID interno: ${internalId})`);
...
function actualizarEstadoEDP(
  ...
  `;
        ...
        // Obtener el ID interno del elemento
        const internalId = item.getAttribute('data-internal-id');
        const nEdp = item.getAttribute('data-n-edp') || edpId;
        
        // Si tenemos ID interno, usar la nueva API, sino usar la antigua
        const useInternalId = internalId && internalId !== '' && internalId !== 'null';
        const apiUrl = useInternalId ? "/dashboard/kanban/update_estado_by_id" : "/dashboard/kanban/update_estado";
        
        const requestBody = useInternalId ? {
        ...
        console.log(`🔄 Actualizando EDP usando ${useInternalId ? 'ID interno' : 'n_edp'}:`, {
            useInternalId,
            internalId,
            nEdp,
            requestBody
...
function mostrarModalContextual(
  ...
  `;
		...
		// 🔍 CRÍTICO: Agregar ID interno si está disponible para evitar problemas de unicidad
		if (item) {
			const internalId = item.getAttribute('data-internal-id');
			if (internalId && internalId !== '' && internalId !== 'null') {
				formData.append("internal_id", internalId);
				console.log(`✅ Enviando con ID interno: ${internalId} (SEGURO)`);
			} else {
				console.warn(`⚠️ EDP ${edpId} sin ID interno - usando n_edp (RIESGOSO)`);
			}
		} else {
			console.warn(`⚠️ No hay elemento item disponible - usando solo n_edp (RIESGOSO)`);
		}
		...
		fetch("/dashboard/kanban/update_estado_detallado", {
			method: "POST",
			body: formData,
		})

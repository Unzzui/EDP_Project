...
class ControllerService(BaseService):
    ...
    def _enriquecer_df_con_estado_detallado(self, df_edp: pd.DataFrame, df_log: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def _obtener_meses_ordenados(self, df: pd.DataFrame) -> list:
        ...
        try:
            ...
            import traceback
            print(f"Error en _obtener_meses_ordenados: {str(e)}")
    ...
    def _calcular_dias_espera(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_calculado['Días Espera'] = None # Initialize with a default
        ...
        for idx, row in df_calculado.iterrows():
            if pd.notna(row.get('Fecha Envío al Cliente')):
                ...
                if row.get('Conformidad Enviada') == 'Sí' and pd.notna(row.get('Fecha Conformidad')):
                    df_calculado.at[idx, 'Días Espera'] = (row['Fecha Conformidad'] - fecha_envio).days
                else:
                    df_calculado.at[idx, 'Días Espera'] = (now - fecha_envio).days
        return df_calculado

    def _calcular_dias_habiles(self, fecha_inicio: Any, fecha_fin: Any) -> float: # Changed return type hint
        """
    ...
    def _calcular_dso_para_dataset(self, df_datos: pd.DataFrame) -> float:
        ...
        required_cols = ["Fecha Emisión", "Monto Aprobado"]
        for col in required_cols:
            if col not in df_datos.columns:
                print(f"WARNING: Columna '{col}' no encontrada en el DataFrame para DSO.")
                return 0
        
        # Ensure 'Monto Aprobado' is numeric
        df_datos['Monto Aprobado'] = pd.to_numeric(df_datos['Monto Aprobado'], errors='coerce').fillna(0)

        # Convert date columns to datetime, coercing errors
        df_datos["Fecha Emisión"] = pd.to_datetime(df_datos["Fecha Emisión"], errors='coerce')
        if "Fecha Pago" in df_datos.columns:
            df_datos["Fecha Pago"] = pd.to_datetime(df_datos["Fecha Pago"], errors='coerce')
        if "Fecha Conformidad" in df_datos.columns:
            df_datos["Fecha Conformidad"] = pd.to_datetime(df_datos["Fecha Conformidad"], errors='coerce')

        fecha_pago_exists = "Fecha Pago" in df_datos.columns
        fecha_conformidad_exists = "Fecha Conformidad" in df_datos.columns
        
        mask_conditions = [~pd.isna(df_datos["Fecha Emisión"])]
        sub_mask_conditions = []

        if fecha_pago_exists:
            sub_mask_conditions.append(~pd.isna(df_datos["Fecha Pago"]))
        if fecha_conformidad_exists:
            sub_mask_conditions.append(~pd.isna(df_datos["Fecha Conformidad"]))
        ...
        if fecha_pago_exists and fecha_conformidad_exists:
            df_calculable["Fecha Final"] = df_calculable.apply(
                lambda x: x["Fecha Pago"] if pd.notna(x["Fecha Pago"]) else x["Fecha Conformidad"], axis=1
            )
    ...
    def _calcular_dso(self, df_datos: pd.DataFrame, mes_anterior_str: Optional[str] = None, df_mes_anterior: Optional[pd.DataFrame] = None) -> Tuple[float, float, list]:
        ...
        """
        dso_global = self._calcular_dso_para_dataset(df_datos.copy()) # Use copy to avoid side effects
        dso_var = 0
        ...
        if df_mes_anterior is not None and not df_mes_anterior.empty:
            ...
            if dso_anterior > 0 and dso_global > 0 : # Avoid division by zero or meaningless variation
        ...
        top_proyectos = []
        # Ensure 'Fecha Emisión' exists for 'Días Pendiente' calculation
        if "Fecha Emisión" not in df_datos.columns:
            print("WARNING: 'Fecha Emisión' no encontrada para calcular top proyectos pendientes en DSO.")
        ...
        if not df_pendientes.empty:
            # Calculate 'Días Pendiente' safely
            df_pendientes["Días Pendiente"] = (pd.Timestamp.now().normalize() - df_pendientes["Fecha Emisión"]).dt.days
            
            # Ensure 'Monto Aprobado' is numeric for sorting and display
            df_pendientes["Monto Aprobado"] = pd.to_numeric(df_pendientes["Monto Aprobado"], errors='coerce').fillna(0)
            ...
            top_proyectos = [
                {
                    ...
                    "dias": int(row["Días Pendiente"]) if pd.notna(row["Días Pendiente"]) else 0, # Ensure dias is int
                    "monto": row.get("Monto Aprobado", 0), # Already ensured numeric
                    "encargado": row.get("Jefe de Proyecto", "")
                }
                for _, row in top_3.iterrows() if pd.notna(row.get("Días Pendiente")) # Ensure 'Días Pendiente' is not NaN
            ]
        return round(dso_global, 1), round(dso_var, 1), top_proyectos

    def _calcular_variaciones_mensuales(self, df_full: pd.DataFrame, mes_actual_param: Optional[str], meses_disponibles: list, total_pagado_global: float) -> Dict[str, float]:
        """
        ...
        # Ensure 'Monto Aprobado' is numeric
        df_full['Monto Aprobado'] = pd.to_numeric(df_full['Monto Aprobado'], errors='coerce').fillna(0)

        mes_actual = mes_actual_param if mes_actual_param else (max(meses_disponibles) if meses_disponibles else None)
        ...
        if mes_actual and len(meses_disponibles) > 1:
            try:
                ...
                mes_anterior_str = meses_disponibles_str[idx_mes_actual - 1] if idx_mes_actual > 0 else None
            except ValueError: # mes_actual might not be in meses_disponibles if it's a future/invalid month
                pass 
        ...
        if mes_anterior_str:
            ...
            if pagado_mes_anterior != 0:
                variaciones["pagado_var_porcentaje"] = round(
                    ((total_pagado_global - pagado_mes_anterior) / pagado_mes_anterior * 100), 1
                )
            elif total_pagado_global > 0 : # Pagado anterior era 0, actual es > 0
                 variaciones["pagado_var_porcentaje"] = 100.0 # Or some large number to indicate change from zero
            ...
            if pendiente_anterior != 0:
                variaciones["pendiente_var_porcentaje"] = round(
                    ((pendiente_actual - pendiente_anterior) / pendiente_anterior * 100), 1
                )
            ...
            avance_global_actual = round(total_pagado_global / META_GLOBAL * 100, 1) if META_GLOBAL > 0 else 0.0
            variaciones["avance_var_porcentaje"] = round(avance_global_actual - avance_mes_anterior, 1)
        
        return variaciones

    def get_controller_dashboard_data(self, filters: Dict[str, Any] = None) -> ServiceResponse:
    ...
    def _apply_manager_filters(self, df: pd.DataFrame, 
        ...
        if filters.get('cliente') and filters['cliente'] != 'todos':
            filtered_df = filtered_df[
                filtered_df['cliente'] == filters['cliente']
            ]
        
        # Status filter
        if filters.get('estado') and filters['estado'] != 'todos':
            filtered_df = filtered_df[
                filtered_df['estado'] == filters['estado']
            ]
    ...
    def _calculate_financial_metrics(self, df_full: pd.DataFrame, 
                                   df_filtered: pd.DataFrame) -> Dict[str, Any]:
        ...
        pending_revenue = df_full[df_full['estado'].isin(['enviado', 'validado'])]['monto_aprobado'].sum()
    ...
    def _calculate_dso_service_internal(self, df: pd.DataFrame) -> float:
        ...
        if "Fecha Emisión" in df.columns and "Monto Aprobado" in df.columns and \
           ("Fecha Pago" in df.columns or "Fecha Conformidad" in df.columns):
            return self._calcular_dso_para_dataset(df.copy())
        return 30.0 # Default placeholder

    def _generate_chart_data(self, df_full: pd.DataFrame, 
                           df_filtered: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            charts['concentracion_clientes'] = self._build_client_performance_chart(df_full)
            ...
            if 'gestor' in df_full.columns or 'jefe_proyecto' in df_full.columns:
                charts['rendimiento_gestores'] = self._build_manager_performance_chart(df_full)
        ...
        try:
            ...
            if 'fecha_estimada_pago' in df_pendientes.columns:
                ...
                if 'fecha_emision' in df_pendientes.columns:
                    df_pendientes['fecha_emision'] = pd.to_datetime(df_pendientes['fecha_emision'], errors='coerce')
                    df_pendientes['Fecha_Estimada_Calculada'] = df_pendientes['fecha_emision'] + pd.Timedelta(days=45)
                    fecha_pago_col = 'Fecha_Estimada_Calculada'
                else: # Not enough data to forecast
                     return {'labels': ['30 días', '60 días', '90 días'], 'datasets': []}

            # Ensure 'monto_aprobado' is numeric
            df_pendientes['monto_aprobado'] = pd.to_numeric(df_pendientes['monto_aprobado'], errors='coerce').fillna(0)
            
            # Calcular montos por período
            monto_30d = df_pendientes[df_pendientes[fecha_pago_col] <= hoy_30d]['monto_aprobado'].sum() / 1_000_000
            monto_60d = df_pendientes[(df_pendientes[fecha_pago_col] > hoy_30d) & \
                                    (df_pendientes[fecha_pago_col] <= hoy_60d)]['monto_aprobado'].sum() / 1_000_000
            monto_90d = df_pendientes[(df_pendientes[fecha_pago_col] > hoy_60d) & \
                                    (df_pendientes[fecha_pago_col] <= hoy_90d)]['monto_aprobado'].sum() / 1_000_000
    ...
    def _build_status_distribution_chart(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            status_mapping = {
                ...
                'revision': {'label': 'Revisión', 'color': 'rgba(245, 158, 11, 0.8)'}, # Original was 'revisión'
                'revisión': {'label': 'Revisión', 'color': 'rgba(245, 158, 11, 0.8)'},
                'pendiente': {'label': 'Pendiente', 'color': 'rgba(239, 68, 68, 0.8)'},
                'rechazado': {'label': 'Rechazado', 'color': 'rgba(107, 114, 128, 0.8)'}
            }
            ...
            for status, count in status_counts.items():
                ...
                mapping = status_mapping.get(status_key, {'label': str(status).title(), 'color': 'rgba(156, 163, 175, 0.8)'})
            ...
            print(f"Error in status distribution chart: {e}")
            return {'labels': [], 'datasets': []}
    ...
    def _build_client_performance_chart(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'cliente' not in df.columns or 'monto_aprobado' not in df.columns or df.empty:
                return {'labels': [], 'datasets': []}
            ...
            client_performance = df.groupby('cliente')['monto_aprobado'].sum() / 1_000_000 # In millions
            ...
            if total_top_10 == 0:
                 pct_acum = pd.Series([0.0] * len(top_10_clientes), index=top_10_clientes.index)
            else:
                ...
                'datasets': [
                    {
                        'type': 'bar',
                        'label': 'Monto (M$)',
                        'data': top_10_clientes.round(1).tolist(), # Use top_10_clientes for data
                        'backgroundColor': [
            ...
            print(f"Error in client performance chart: {e}")
            return {'labels': [], 'datasets': []}
    ...
    def _build_monthly_trend_chart(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'fecha_emision' not in df_copia.columns or 'monto_aprobado' not in df_copia.columns:
                return self._create_mock_monthly_trend()

            df_copia['fecha_emision'] = pd.to_datetime(df_copia['fecha_emision'], errors='coerce')
            df_copia['monto_aprobado'] = pd.to_numeric(df_copia['monto_aprobado'], errors='coerce').fillna(0)
            ...
            try:
                ...
                if cost_response.get('success', False):
                    ...
                    if not df_costs.empty:
                        ...
                        date_col_options = ['fecha_costo', 'created_at', 'fecha_emision'] # ordered preference
                        cost_date_col_found = False
                        for col_name in date_col_options:
                            if col_name in df_costs.columns:
                                df_costs[col_name] = pd.to_datetime(df_costs[col_name], errors='coerce')
                                df_costs['mes'] = df_costs[col_name].dt.strftime('%Y-%m')
                                cost_date_col_found = True
                                break
                        if not cost_date_col_found: # Fallback if no date column
                             df_costs['mes'] = datetime.now().strftime('%Y-%m')
                        print(f"✅ Loaded {len(df_costs)} cost records for trend analysis")
            except Exception as e:
                print(f"⚠️ Could not load cost data for trend: {e}")
            
            df_completados = df_copia[df_copia['estado'].str.strip().isin(['pagado', 'validado'])]
            ingresos_mensuales = df_completados.groupby('mes')['monto_aprobado'].sum() / 1_000_000
            ingresos_emitidos = df_copia.groupby('mes')['monto_aprobado'].sum() / 1_000_000
            
            costos_mensuales = pd.Series(dtype=float)
            if not df_costs.empty and 'mes' in df_costs.columns and 'importe_neto' in df_costs.columns:
                costos_mensuales = df_costs.groupby('mes')['importe_neto'].sum() / 1_000_000
            
            todos_meses = set()
            if not ingresos_mensuales.empty: todos_meses.update(ingresos_mensuales.index)
            if not ingresos_emitidos.empty: todos_meses.update(ingresos_emitidos.index)
            if not costos_mensuales.empty: todos_meses.update(costos_mensuales.index)
            
            if not todos_meses: return self._create_mock_monthly_trend()
            
            meses_ordenados = sorted(list(todos_meses)) # Ensure it's a list for slicing
            ...
            if meses_ordenados: # Check if meses_ordenados is not empty
                ...
                for i in range(1, 4):
                    mes_futuro = ultimo_mes_dt + pd.DateOffset(months=i) # Use DateOffset for month increments
                    meses_proyeccion.append(mes_futuro.strftime('%Y-%m'))
            
            periodo_completo = ultimos_6_meses + meses_proyeccion
            
            labels, datos_ingresos_reales, datos_ingresos_emitidos, datos_costos, datos_margen, datos_cashflow_proyectado = [], [], [], [], [], []
            
            avg_ingresos_reales = ingresos_mensuales.tail(3).mean() if len(ingresos_mensuales) >= 3 else (ingresos_mensuales.mean() if not ingresos_mensuales.empty else 0)
            avg_ingresos_emitidos = ingresos_emitidos.tail(3).mean() if len(ingresos_emitidos) >= 3 else (ingresos_emitidos.mean() if not ingresos_emitidos.empty else 0)
            ...
            if not costos_mensuales.empty:
                avg_costos = costos_mensuales.tail(3).mean() if len(costos_mensuales) >=3 else costos_mensuales.mean()
            elif avg_ingresos_reales > 0 : # Base cost projection on revenue if no cost data
            ...
            for i, mes_str in enumerate(periodo_completo):
                try:
                    ...
                    labels.append(fecha.strftime('%b %Y') + (' (P)' if i >= len(ultimos_6_meses) else ''))
                    ...
                    if not es_proyeccion:
                        ...
                        cost_real = avg_costos * factor # Project costs based on avg_costos
                    ...
                    datos_cashflow_proyectado.append(margen) # Simplified: cashflow = margin for the month
                except Exception as e_mes:
                    print(f"Error procesando mes {mes_str} en trend chart: {e_mes}")
                    # Append defaults to maintain list lengths
                    labels.append(mes_str)
                    for lst in [datos_ingresos_reales, datos_ingresos_emitidos, datos_costos, datos_margen, datos_cashflow_proyectado]:
                        lst.append(0)

            datasets = [
                {'label': 'Ingresos Cobrados (M$)', 'data': datos_ingresos_reales, 'borderColor': '#10B981', 'backgroundColor': 'rgba(16, 185, 129, 0.1)', 'fill': True, 'tension': 0.3, 'borderWidth': 3, 'pointBackgroundColor': '#10B981', 'pointBorderWidth': 2, 'pointRadius': 4},
                {'label': 'Ingresos Proyectados (M$)', 'data': datos_ingresos_emitidos, 'borderColor': '#3B82F6', 'backgroundColor': 'rgba(59, 130, 246, 0.1)', 'fill': False, 'tension': 0.3, 'borderWidth': 2, 'borderDash': [5, 5], 'pointBackgroundColor': '#3B82F6', 'pointRadius': 3},
                {'label': 'Costos (M$)', 'data': datos_costos, 'borderColor': '#F87171', 'backgroundColor': 'rgba(248, 113, 113, 0.1)', 'fill': False, 'tension': 0.3, 'borderWidth': 2, 'pointBackgroundColor': '#F87171', 'pointRadius': 3},
                {'label': 'Margen Bruto (M$)', 'data': datos_margen, 'borderColor': '#8B5CF6', 'backgroundColor': 'rgba(139, 92, 246, 0.2)', 'fill': True, 'tension': 0.3, 'borderWidth': 2, 'pointBackgroundColor': '#8B5CF6', 'pointRadius': 3}
            ]
            
            margen_real_reciente = [m for i, m in enumerate(datos_margen) if i < len(ultimos_6_meses) and pd.notna(m)] # Filter out NaN
            promedio_margen = sum(margen_real_reciente[-3:]) / 3 if len(margen_real_reciente) >= 3 else (sum(margen_real_reciente) / len(margen_real_reciente) if margen_real_reciente else 0)
            
            tendencia_margen = 0
            if len(margen_real_reciente) >= 2 and margen_real_reciente[0] != 0:
                tendencia_margen = ((margen_real_reciente[-1] - margen_real_reciente[0]) / margen_real_reciente[0] * 100)
            
            cashflow_3m = sum(d for d in datos_cashflow_proyectado[-3:] if pd.notna(d)) # Sum only non-NaN
            ...
            return {
                ...
                'estadisticas': {
                    ...
                    'mejor_mes': labels[datos_margen.index(max(m for m in datos_margen if pd.notna(m)))] if any(pd.notna(m) for m in datos_margen) else 'N/A',
    ...
    def _build_aging_buckets_chart(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'dias_espera' not in df.columns or df.empty:
                return {'labels': ['0-30 días', '31-60 días', '61-90 días', '90+ días'], 'datasets': []}
            
            # Make a copy to avoid SettingWithCopyWarning if df is a slice
            df_copy = df.copy()
            df_copy['dias_espera'] = pd.to_numeric(df_copy['dias_espera'], errors='coerce').fillna(0)
            ...
            return {
                ...
                'datasets': [{'label': 'EDPs por antigüedad',
                              'data': [bucket_0_30, bucket_31_60, bucket_61_90, bucket_90_plus],
                              'backgroundColor': ['rgba(16, 185, 129, 0.8)', 'rgba(59, 130, 246, 0.8)', 
                                                'rgba(249, 115, 22, 0.8)', 'rgba(239, 68, 68, 0.8)']
                             }]
            }
        except Exception as e:
            print(f"Error in aging buckets chart: {e}")
            return {'labels': ['0-30 días', '31-60 días', '61-90 días', '90+ días'], 'datasets': []}
    ...
    def load_related_data(self) -> ServiceResponse:
        ...
        try:
            # Get EDP data
            edps_response = self.edp_repo.find_all()
            logs_response = self.log_repository.find_all()
            ...
            if isinstance(logs_response, dict) and not logs_response.get('success', False):
                return ServiceResponse(
                    success=False,
    ...
    def _build_manager_performance_chart(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            manager_col = 'jefe_proyecto' if 'jefe_proyecto' in df.columns else ('gestor' if 'gestor' in df.columns else None)
            if not manager_col or df.empty: return {'labels': ['Sin datos'], 'datasets': []}

            # Ensure required columns are present and numeric
            if 'monto_aprobado' not in df.columns: return {'labels': ['Sin datos de monto'], 'datasets': []}
            df_copy = df.copy() # Avoid SettingWithCopyWarning
            df_copy['monto_aprobado'] = pd.to_numeric(df_copy['monto_aprobado'], errors='coerce').fillna(0)
            ...
            try:
                ...
                if cost_response.get('success', False):
                    df_costs = cost_response.get('data', pd.DataFrame())
                    if not df_costs.empty and 'project_id' in df_costs.columns and 'importe_neto' in df_costs.columns:
                        df_costs['project_id'] = df_costs['project_id'].astype(str).str.strip()
                        df_costs['importe_neto'] = pd.to_numeric(df_costs['importe_neto'], errors='coerce').fillna(0)
                        
                        df_costs_valid = df_costs[
                            (df_costs['project_id'].notna()) & (df_costs['project_id'] != '') & (df_costs['project_id'] != 'nan')
                        ]
                ...
                print(f"Warning: Could not load/process cost data for manager performance: {e_cost}")
            
            gestores_data = {}
            for gestor_name in df_copy[manager_col].dropna().unique():
                if not pd.notna(gestor_name) or not str(gestor_name).strip(): continue
                ...
                for _, row in df_gestor.iterrows():
                    ...
                    proyecto_id_options = [str(row.get('proyecto','')).strip(), str(row.get('n_edp','')).strip()]
                    proyecto_id = next((pid for pid in proyecto_id_options if pid), None) # Get first valid ID

                    ingreso_proyecto = row['monto_aprobado'] # Already numeric
                    
                    if str(row.get('estado', '')).strip().lower() in ['pagado', 'validado']:
                        ingresos_totales += ingreso_proyecto
                        if proyecto_id and proyecto_id in costs_lookup:
                            costos_totales += costs_lookup[proyecto_id].get('importe_neto', 0)
                        proyectos_procesados +=1 # Count project if it's pagado/validado

                if ingresos_totales > 0: # Avoid division by zero for rentabilidad
                    margen = ingresos_totales - costos_totales
                    rentabilidad = (margen / ingresos_totales * 100) if ingresos_totales != 0 else 0
                    gestores_data[gestor_name] = {
                        'rentabilidad': rentabilidad, 'ingresos': ingresos_totales / 1_000_000,
                        'margen': margen / 1_000_000, 'proyectos': proyectos_procesados
                    }
            
            if not gestores_data: return {'labels': ['Sin coincidencias'], 'datasets': []}
            
            top_gestores = dict(sorted(gestores_data.items(), key=lambda x: x[1]['rentabilidad'], reverse=True)[:8])
            ...
            for gestor, data in top_gestores.items():
                ...
                if rent >= 35: colores.append('rgba(16, 185, 129, 0.8)')   # Excelente
                elif rent >= 25: colores.append('rgba(34, 197, 94, 0.7)')  # Bueno
                elif rent >= 15: colores.append('rgba(59, 130, 246, 0.7)')  # Aceptable
                elif rent >= 0: colores.append('rgba(249, 115, 22, 0.7)')   # Bajo
                else: colores.append('rgba(239, 68, 68, 0.8)')            # Negativo

            tooltip_data_dict = {
                str(g): {'rentabilidad': f"{d['rentabilidad']:.1f}%", 'ingresos': f"${d['ingresos']:.1f}M",
                         'margen': f"${d['margen']:.1f}M", 'proyectos': f"{d['proyectos']} proyectos"}
                for g, d in top_gestores.items()
            }

            return {
                'labels': labels,
                'datasets': [{'label': 'Rentabilidad por Gestor (%)', 'data': rentabilidades_data, 
                              'backgroundColor': colores, 
                              'borderColor': [c.replace('0.7', '1.0').replace('0.8', '1.0') for c in colores],
                              'borderWidth': 1}],
                'tooltip_data': tooltip_data_dict
            }
        except Exception as e:
            print(f"Error in manager performance chart: {e}")
            import traceback
            traceback.print_exc()
            return {'labels': [], 'datasets': []}
   
    def _build_opex_capex_chart(self, df: pd.DataFrame) -> Dict[str, Any]: # df here is edp_df, not costs_df
        try:
            cost_response = self.cost_service.cost_repository.find_all_dataframe()
            ...
            df_costs = cost_response.get('data', pd.DataFrame())
            if df_costs.empty or 'tipo_costo' not in df_costs.columns or 'importe_neto' not in df_costs.columns:
                 return {'labels': ['OPEX', 'CAPEX'], 'datasets': [{'data': [0,0]}]} # Default empty structure

            df_costs['tipo_costo'] = df_costs['tipo_costo'].str.lower().fillna('opex')
            df_costs['importe_neto'] = pd.to_numeric(df_costs['importe_neto'], errors='coerce').fillna(0)

            analisis_tipo = df_costs.groupby('tipo_costo')['importe_neto'].agg(total='sum').reset_index()
            
            # Ensure both opex and capex are present, even if zero
            opex_total = analisis_tipo[analisis_tipo['tipo_costo'] == 'opex']['total'].sum() / 1_000_000
            capex_total = analisis_tipo[analisis_tipo['tipo_costo'] == 'capex']['total'].sum() / 1_000_000


            return {
                'labels': ['OPEX', 'CAPEX'], # Fixed labels
                'datasets': [{'data': [round(opex_total,2), round(capex_total,2)],
            ...
            import traceback
            print(f"Error in _build_opex_capex_chart: {e}")
            traceback.print_exc()
    ...
    def _create_mock_monthly_trend(self) -> Dict[str, Any]:
        ...
        from datetime import datetime, timedelta
        import random
        months = [(datetime.now() - timedelta(days=30 * i)).strftime('%Y-%m') for i in range(6, 0, -1)]
        ...
        return {
            'labels': months,
            'datasets': [
                {'label': 'Ingresos (M$)', 'data': amounts, 'borderColor': 'rgba(16, 185, 129, 1)', 'backgroundColor': 'rgba(16, 185, 129, 0.2)', 'tension': 0.4, 'fill': True},
                {'label': 'Cantidad EDPs', 'data': counts, 'borderColor': 'rgba(59, 130, 246, 1)', 'backgroundColor': 'rgba(59, 130, 246, 0.2)', 'tension': 0.4, 'yAxisID': 'y1'}
            ]
        }
    ...
    def _calculate_cash_forecast(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if df.empty or not all(col in df.columns for col in ['estado', 'fecha_estimada_pago', 'monto_aprobado']):
            return {'months': ['Oct', 'Nov', 'Dic'], 'amounts': [10, 15, 20], 'total_expected': 45} # Mock

        df_copy = df.copy()
        df_copy['fecha_estimada_pago'] = pd.to_datetime(df_copy['fecha_estimada_pago'], errors='coerce')
        df_copy['monto_aprobado'] = pd.to_numeric(df_copy['monto_aprobado'], errors='coerce').fillna(0)

        pending_payments = df_copy[
            (df_copy['estado'].isin(['validado', 'enviado'])) & (pd.notna(df_copy['fecha_estimada_pago']))
        ]
        
        if not pending_payments.empty:
            pending_payments['Mes_Pago'] = pending_payments['fecha_estimada_pago'].dt.strftime('%Y-%m')
            monthly_forecast = pending_payments.groupby('Mes_Pago')['monto_aprobado'].sum() / 1_000_000 # In Millions
            
            # Format for display (e.g., last 3-6 months or next 3-6 months)
            # For simplicity, let's take up to 6 future months if available
            future_months_forecast = monthly_forecast[monthly_forecast.index >= datetime.now().strftime('%Y-%m')].head(6)
            ...
            return {'months': ['Oct', 'Nov', 'Dic'], 'amounts': [10, 15, 20], 'total_expected': round(45.0,1)}
    ...
    def _generate_alerts(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        if 'dias_espera' in df.columns and 'estado' in df.columns:
            ...
            old_pending_df = df[
                (df['estado'].isin(['enviado', 'revisión', 'pendiente'])) & (df['dias_espera'] > 30)
            ]
            old_pending_count = len(old_pending_df)
            if old_pending_count > 0:
                alerts.append({'type': 'error', 'title': 'EDPs Atrasados', 
    ...
    def get_empty_kpis(self) -> Dict[str, Any]:
        ...
        return {
            "total_edp_global": 0, "total_validados_global": 0, "dias_espera_promedio_global": 0,
            "porcentaje_validacion_rapida_global": 0, "total_filtrados": 0, "total_criticos_filtrados": 0,
            "dias_espera_promedio_filtrado": 0, "dias_habiles_promedio_filtrado": 0,
            "total_pagado_global": 0, "total_propuesto_global": 0, "total_aprobado_global": 0,
            "meta_global": META_GLOBAL, "diferencia_montos": 0, "porcentaje_diferencia": 0,
            "avance_global": 0, "meta_var_porcentaje": 0, "pagado_var_porcentaje": 0,
            "pendiente_var_porcentaje": 0, "avance_var_porcentaje": 0,
            "meta_por_encargado": 0, "monto_pagado_encargado": 0, "avance_encargado": 0,
            "monto_pendiente_encargado": 0, "total_con_conformidad": 0, "porcentaje_conformidad": 0,
            "tiempo_promedio_conformidad": 0, "total_retrabajos": 0, "porcentaje_retrabajos": 0,
            "motivos_rechazo": {}, "tipos_falla": {},
            "total_edp_pagados_conformados": 0, "total_pagado_global_kpi": 0,
            "pagado_reciente": 0, "pagado_medio": 0, "pagado_critico": 0,
            "total_edps_por_cobrar": 0, "total_pendiente_por_cobrar": 0,
            "pendiente_reciente": 0, "pendiente_medio": 0, "pendiente_critico": 0,
            "dso_global": 0, "dso_var": 0, "top_dso_proyectos": [],
            "dso_filtrado": 0, "total_pendiente_filtrado": 0, "total_pagado_filtrado": 0
        }
    ...
    def _prepare_kpi_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df.empty: # Handle empty DataFrame input
            ...
            expected_cols = [
                'Fecha Emisión', 'Fecha Envío al Cliente', 'Fecha Conformidad', 'Fecha Pago', 'Fecha Estimada de Pago',
                'Monto Propuesto', 'Monto Aprobado', 'Validado', 'Crítico', 'Conformidad Enviada',
                'Días Espera', 'Días Hábiles', 'Estado', 'Mes', 'Cliente', 'Jefe de Proyecto', 
                'Estado Detallado', 'Motivo No-aprobado', 'Tipo_falla', 'N° EDP', 'Proyecto' # Added N° EDP, Proyecto
            ]
        ...
        date_cols = ['Fecha Emisión', 'Fecha Envío al Cliente', 'Fecha Conformidad', 'Fecha Pago', 'Fecha Estimada de Pago']
        ...
        money_cols = ['Monto Propuesto', 'Monto Aprobado']
        for col in money_cols:
            if col in df_prepared.columns:
                df_prepared[col] = pd.to_numeric(df_prepared[col], errors='coerce').fillna(0.0)
            else:
                df_prepared[col] = 0.0
        ...
        for col, default_val in bool_cols_map.items():
            if col in df_prepared.columns:
                if df_prepared[col].dtype == 'object':
                     df_prepared[col] = df_prepared[col].astype(str).str.lower().map({
                         'sí': True, 'true': True, '1': True, 'yes': True, 'verdadero': True,
                         'no': False, 'false': False, '0': False, 'nan': False, '': False, None: False
                     }).fillna(default_val)
                elif pd.api.types.is_numeric_dtype(df_prepared[col]): # Handle 1/0 as int/float
                    df_prepared[col] = df_prepared[col].map({1: True, 0: False}).fillna(default_val)
        ...
        if 'Fecha Envío al Cliente' in df_prepared.columns and 'Fecha Conformidad' in df_prepared.columns:
            df_prepared['Días Hábiles'] = df_prepared.apply(
                lambda row: self._calcular_dias_habiles(row.get('Fecha Envío al Cliente'), row.get('Fecha Conformidad')),
                axis=1
            )
        ...
        df_prepared['Días Hábiles'] = pd.to_numeric(df_prepared['Días Hábiles'], errors='coerce')


        ensure_cols_with_defaults = {
            'Estado': '', 'Mes': '', 'Cliente': '', 'Jefe de Proyecto': '', 
            'Estado Detallado': '', 'Motivo No-aprobado': '', 'Tipo_falla': '',
            'N° EDP': '', 'Proyecto': '' # Added N° EDP, Proyecto
        }
    ...
    def _calculate_financial_kpis(self, df: pd.DataFrame) -> Dict[str, Any]: return {}
    def _calculate_operational_kpis(self, df: pd.DataFrame) -> Dict[str, Any]: return {}
    def _calculate_profitability_kpis(self, df: pd.DataFrame) -> Dict[str, Any]: return {}
    def _calculate_aging_kpis(self, df: pd.DataFrame) -> Dict[str, Any]: return {}
    def _calculate_efficiency_kpis(self, df: pd.DataFrame) -> Dict[str, Any]: return {}
    def _calculate_monthly_financials(self, df: pd.DataFrame) -> Dict[str, Any]: return {}
    def _calculate_working_capital_impact(self, df: pd.DataFrame) -> float: return 0.0
    def _get_manager_filter_options(self, df: pd.DataFrame) -> Dict[str, Any]: return {}
    def _calculate_performance_by_period(self, df: pd.DataFrame, period: str) -> Dict[str, Any]: return {}
    def _calculate_trends(self, df: pd.DataFrame, period: str) -> Dict[str, Any]: return {}
    def _generate_recommendations(self, perf_data: Dict, trends: Dict) -> List[str]: return []

    def _get_default_processed_context(self, request_filters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def get_processed_dashboard_context(self, df_edp_raw: pd.DataFrame, df_log_raw: pd.DataFrame, request_filters: Dict[str, Any]) -> ServiceResponse:
        """
        ...
        try:
            ...
            df_full = self._enriquecer_df_con_estado_detallado(df_full_prepared, df_log_raw.copy() if df_log_raw is not None else pd.DataFrame())
            ...
            estados_detallados_unicos = sorted(list(df_full["Estado Detallado"].dropna().unique())) if "Estado Detallado" in df_full.columns else []
            ...
            # Estado Detallado filter (applied before Estado filter, as in original)
            if estado_detallado_filter:
                df_filtered = df_filtered[df_filtered["Estado Detallado"] == estado_detallado_filter]
            ...
            dias_espera_promedio_global_raw = df_full["Días Espera"].mean()
            dias_espera_promedio_global = round(dias_espera_promedio_global_raw if pd.notna(dias_espera_promedio_global_raw) else 0, 1)
            
            validados_rapidos_global = df_full[(df_full["Validado"] == True) & (df_full["Días Espera"] <= 30)].shape[0]
            ...
            dias_espera_promedio_filtrado_raw = df_filtered["Días Espera"].mean()
            ...
            dias_habiles_promedio_filtrado_raw = df_filtered["Días Hábiles"].mean() # Días Hábiles is float/np.nan
            dias_habiles_promedio_filtrado = round(dias_habiles_promedio_filtrado_raw if pd.notna(dias_habiles_promedio_filtrado_raw) else 0, 1)
            kpis_filtrados = {
            ...
            porcentaje_diferencia = round((diferencia_montos / total_propuesto_global * 100), 1) if total_propuesto_global > 0 else 0.0
            avance_global = round(total_pagado_global / META_GLOBAL * 100, 1) if META_GLOBAL > 0 else 0.0
            ...
            total_pendiente_filtrado = df_filtered[df_filtered["Estado"].isin(["enviado", "pendiente", "revisión"])]["Monto Propuesto"].sum()
            total_pagado_filtrado = df_filtered[df_filtered["Estado"].isin(["pagado", "validado"])]["Monto Aprobado"].sum()
            ...
            if encargado_filter: # df_filtered is already filtered by encargado if filter is active
                monto_pagado_encargado = df_filtered[df_filtered["Estado"] == "pagado"]["Monto Aprobado"].sum()
                # Original used Monto Aprobado for pendiente_por_pago_encargado
                pendiente_por_pago_encargado = df_filtered[df_filtered["Estado"].isin(["pendiente", "revisión", "enviado"])]["Monto Aprobado"].sum()
            ...
            mask_fechas_validas = df_full["Fecha Envío al Cliente"].notna() & df_full["Fecha Conformidad"].notna()
            if mask_fechas_validas.any():
                tiempos_conformidad = (df_full.loc[mask_fechas_validas, "Fecha Conformidad"] - df_full.loc[mask_fechas_validas, "Fecha Envío al Cliente"]).dt.days
                ...
                if not tiempos_conformidad_positivos.empty:
                     tiempo_promedio_conformidad_raw = tiempos_conformidad_positivos.mean()
                     tiempo_promedio_conformidad = round(tiempo_promedio_conformidad_raw if pd.notna(tiempo_promedio_conformidad_raw) else 0,1)
            ...
            analisis_retrabajos = {
                ...
                "motivos_rechazo": motivos_rechazo_series.to_dict() if not motivos_rechazo_series.empty else {},
                "tipos_falla": tipos_falla_series.to_dict() if not tipos_falla_series.empty else {}
            }
            ...
            pagos_data = {
                ...
                "total_pagado_global_kpi": edps_pagados_conformados_df["Monto Aprobado"].sum(),
                "pagado_reciente": edps_pagados_conformados_df[edps_pagados_conformados_df["Días Espera"] <= 30]["Monto Aprobado"].sum(),
                "pagado_medio": edps_pagados_conformados_df[(edps_pagados_conformados_df["Días Espera"] > 30) & (edps_pagados_conformados_df["Días Espera"] <= 60)]["Monto Aprobado"].sum(),
                "pagado_critico": edps_pagados_conformados_df[edps_pagados_conformados_df["Días Espera"] > 60]["Monto Aprobado"].sum()
            }
            ...
            pendientes_data = {
                ...
                "total_pendiente_por_cobrar": edps_por_cobrar_df["Monto Propuesto"].sum(), # Original uses Monto Propuesto here
                "pendiente_reciente": edps_por_cobrar_df[edps_por_cobrar_df["Días Espera"] <= 30]["Monto Propuesto"].sum(),
                "pendiente_medio": edps_por_cobrar_df[(edps_por_cobrar_df["Días Espera"] > 30) & (edps_por_cobrar_df["Días Espera"] <= 60)]["Monto Propuesto"].sum(),
                "pendiente_critico": edps_por_cobrar_df[edps_por_cobrar_df["Días Espera"] > 60]["Monto Propuesto"].sum()
            }
            ...
            dso_global, dso_var, top_dso_proyectos_raw = self._calcular_dso(
                df_full.copy(), 
                mes_anterior_para_dso, 
                df_mes_anterior_para_dso
            )

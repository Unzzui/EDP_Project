...
class AnalyticsService:
    ...
    def get_basic_stats(self) -> ServiceResponse:
        ...
        try:
            ...
            tasa_aprobacion = 0
            if 'estado' in df.columns:
                total_procesados = len(df[df['estado'].isin(['pagado', 'validado', 'enviado'])])
                pagados_validados = len(df[df['estado'].isin(['pagado', 'validado'])])
                if total_procesados > 0:
                    tasa_aprobacion = round((pagados_validados / total_procesados) * 100, 1)
    ...
    def _enriquecer_log_con_edp(self, df_log: pd.DataFrame, df_edp: pd.DataFrame) -> pd.DataFrame:
        ...
        df_enriquecido = pd.merge(
            df_log,
            df_edp[["n_edp", "proyceto", "jefe_proyecto", "cliente", "mes", 
                   "tipo_falla", "motivo_no_aprobado", "monto_aprobado"]],
            on="n_edp",
            how="left"
        )
    ...
    def _analizar_financiero_encargado(self, df_encargado: pd.DataFrame, nombre: str) -> Dict:
        ...
        df_pendientes = df_encargado[~df_encargado["estado"].isin(["pagado", "validado"])]
        
        distribucion_aging = {
            'reciente': len(df_pendientes[df_pendientes["dias_espera"] <= 15]),
            'medio': len(df_pendientes[(df_pendientes["dias_espera"] > 15) & (df_pendientes["dias_espera"] <= 30)]),
            'critico': len(df_pendientes[df_pendientes["dias_espera"] > 30])
        }
        
        # Calculate amounts by aging buckets
        montos_aging = {
            'reciente': df_pendientes[df_pendientes["dias_espera"] <= 15]["monto_aprobado"].sum(),
            'medio': df_pendientes[(df_pendientes["dias_espera"] > 15) & (df_pendientes["dias_espera"] <= 30)]["monto_aprobado"].sum(),
            'critico': df_pendientes[df_pendientes["dias_espera"] > 30]["monto_aprobado"].sum()
        }
        
        # Calculate efficiency metrics
        tiempo_promedio_resolucion = df_pagados["dias_espera"].mean() if len(df_pagados) > 0 else 0
        ...
        if total_edps > 0:
            riesgo_score = (
                ...
                (max(0, (tiempo_promedio_resolucion - 30)) / 60 * 30) +  # Time over 30 days weight: 30%
                (max(0, (100 - tasa_aprobacion)) / 100 * 30)  # Low approval rate weight: 30%
            )
    ...
    def _analizar_rendimiento_encargado(self, df_encargado: pd.DataFrame, df_global: pd.DataFrame) -> Dict:
        ...
        if len(df_global) > 0:
            df_global_pagados = df_global[df_global["estado"].isin(["pagado", "validado"])]
        ...
        dias_promedio_aprobacion = df_con_fechas["dias_espera"].mean() if len(df_con_fechas) > 0 else 0
        ...
        if "mes" in df_encargado.columns:
            ...
            velocidad_procesamiento = edps_por_mes.mean() if len(edps_por_mes) > 0 else 0
            
            # Calculate monthly payment velocity
            pagos_por_mes = df_encargado[df_encargado["estado"].isin(["pagado", "validado"])].groupby("mes")["monto_aprobado"].sum()
        ...
        meses_unicos = sorted(df_encargado["mes"].unique()) if "mes" in df_encargado.columns else []
        ...
        if len(meses_unicos) >= 2:
            ...
            dso_ultimo = self._calcular_dso_dataset(df_encargado[df_encargado["mes"] == ultimo_mes])
            dso_penultimo = self._calcular_dso_dataset(df_encargado[df_encargado["mes"] == penultimo_mes])
    ...
    def _analizar_tendencias_encargado(self, df_encargado: pd.DataFrame) -> Dict:
        ...
        for mes in meses_unicos:
            df_mes = df_encargado[(df_encargado["mes"] == mes) & (df_encargado["estado"].isin(["pagado", "validado"]))]
        ...
        if monto_cobrado_penultimo_mes > 0:
            variacion_mensual_cobro = ((monto_cobrado_ultimo_mes - monto_cobrado_penultimo_mes) / monto_cobrado_penultimo_mes * 100)
        ...
        meta_mes_actual = 375_000_000   # Monthly target (assuming annual target divided by 12)
        ...
        if tendencia_cobro:
            ...
            if len(tendencia_cobro) >= 2:
                ...
                proyeccion_siguiente_mes = monto_cobrado_ultimo_mes + crecimiento_promedio
            else:
        ...
        if len(meses_unicos) >= 3:
            ...
            ultimos_3_meses = [tendencia_cobro[i][1] for i in range(-3, 0)] if len(tendencia_cobro) >= 3 else []
            if len(ultimos_3_meses) == 3 and ultimos_3_meses[0] > ultimos_3_meses[1] > ultimos_3_meses[2]:
                alertas.append({
                    'tipo': 'danger',
        ...
        if promedio_cobro_mensual > 0 and meta_mes_actual > 0:
            cumplimiento_meta = min(100, (promedio_cobro_mensual / meta_mes_actual) * 100)
            consistency_score = 100 - abs(variacion_mensual_cobro)  # Lower variation is better

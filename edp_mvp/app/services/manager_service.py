...
class ManagerService(BaseService):
    ...
    def get_manager_dashboard_data(self, filters: Dict[str, Any] = None) -> ServiceResponse:
    ...
    def _build_status_distribution_chart(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for status, count in status_counts.items():
                mapping = status_mapping.get(status, {'label': status.title(), 'color': 'rgba(156, 163, 175, 0.8)'})
    ...
    def _build_client_performance_chart(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            client_performance = df.groupby('cliente')['monto_aprobado'].sum() / 1_000_000
                ...
                if cost_response.get('success', False):
                    ...
                    if not df_costs.empty:
                        ...
                        if 'fecha_costo' in df_costs.columns:
                            df_costs['fecha_costo'] = pd.to_datetime(df_costs['fecha_costo'], errors='coerce')
                            df_costs['mes'] = df_costs['fecha_costo'].dt.strftime('%Y-%m')
                        elif 'created_at' in df_costs.columns:
                            df_costs['created_at'] = pd.to_datetime(df_costs['created_at'], errors='coerce')
                            df_costs['mes'] = df_costs['created_at'].dt.strftime('%Y-%m')
                        else:
                            # Si no hay fecha de costo, usar fecha de emisión del EDP correspondiente
                            if 'project_id' in df_costs.columns:
                                proyecto_fechas = df_copia[['proyecto', 'fecha_emision']].drop_duplicates()
                                proyecto_fechas.columns = ['project_id', 'fecha_emision'] 
                                df_costs = df_costs.merge(proyecto_fechas, on='project_id', how='left')
                                df_costs['mes'] = df_costs['fecha_emision'].dt.strftime('%Y-%m')
                            else:
            ...
            for gestor in df[manager_col].dropna().unique():
                if pd.notna(gestor) and str(gestor).strip():
                    ...
                    for _, row in df_gestor.iterrows():
                        proyecto_id = str(row.get('proyecto', row.get('n_edp', ''))).strip()
    ...
    def _calculate_cash_forecast(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if not pending_payments.empty:
            ...
            monthly_forecast = pending_payments.groupby('Mes_Pago')['monto_aprobado'].sum()
    ...
    def _get_manager_filter_options(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        if 'jefe_proyecto' in df.columns:
            options['jefes_proyecto'] = sorted(df['jefe_proyecto'].dropna().unique().tolist())
    ...
    def _calculate_monthly_financials(self, df: pd.DataFrame) -> Dict[str, List[float]]:
        ...
        paid_by_month = df[df['estado'] == 'pagado'].groupby('mes')['monto_aprobado'].sum()
    ...
    def _calculate_dso(self, df: pd.DataFrame) -> float:
        ...
        if total_amount > 0:
            weighted_dso = (paid_edps['dias_espera'] * paid_edps['monto_aprobado']).sum() / total_amount
            return weighted_dso
    ...
    def _calculate_working_capital_impact(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        pending_amount = df[df['estado'].isin(['enviado', 'validado'])]['monto_aprobado'].sum()
        ...
        if 'dias_espera' in df.columns:
            ...
            daily_cost = (pending_edps['monto_aprobado'] * pending_edps['dias_espera'] * cost_of_capital_daily).sum()
        else:
            daily_cost = 0
    ...
    def _calculate_performance_by_period(self, df: pd.DataFrame, 
        ...
        return {
            ...
            'average_days': df['dias_espera'].mean() if 'dias_espera' in df.columns else 0
        }
    ...
    def _generate_recommendations(self, performance_data: Dict[str, Any], 
        ...
        if performance_data.get('average_days', 0) > 30:
            recommendations.append("Considerar optimizar el proceso de aprobación para reducir tiempos de espera")
        
        if trends.get('direction') == 'down':
            recommendations.append("Implementar medidas correctivas para mejorar la tendencia de aprobación")
    ...
    def calculate_executive_kpis(self, datos_relacionados: Dict[str, Any], filters: Dict[str, Any]) -> ServiceResponse:
    ...
    def generate_executive_charts(self, datos_relacionados: Dict[str, Any], filters: Dict[str, Any]) -> ServiceResponse:
    ...
    def analyze_profitability(self, datos_relacionados: Dict[str, Any], filters: Dict[str, Any]) -> ServiceResponse:
        ...
        try:
            ...
            if cost_data_response.success and cost_data_response.data:
                ...
                try:
                    costs_response = self.cost_service.cost_repository.find_all_dataframe()
            ...
            proyectos_analysis = self._analyze_profitability_by_projects(df_edp, df_costs, costs_available)
            
            # Analyze profitability by clients
            clientes_analysis = self._analyze_profitability_by_clients(df_edp, df_costs, costs_available)
            
            # Analyze profitability by managers
            gestores_analysis = self._analyze_profitability_by_managers(df_edp, df_costs, costs_available)
            ...
            print(f"Error in analyze_profitability: {str(e)}")
            import traceback
            traceback.print_exc()
            return ServiceResponse(
                success=False,
    ...
    def get_top_edps(self, datos_relacionados: Dict[str, Any], limit: int = 10) -> ServiceResponse:
        ...
        try:
            ...
            for _, row in df_sorted.iterrows():
                edp_data = {
                    ...
                    'monto': float(row.get('monto_aprobado', 0)) / 1000000 if pd.notna(row.get('monto_aprobado')) else 0,  # Convert to millions
                    'dias_espera': int(row.get('dias_espera', 0)) if pd.notna(row.get('dias_espera')) else 0,
            ...
            print(f"Error in get_top_edps: {str(e)}")
            import traceback
            traceback.print_exc()
            return ServiceResponse(
                success=False,
                message=f"Error getting top EDPs: {str(e)}",
                data=[]
            )
    ...
    def get_cost_management_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if overdue_count > 0:
                ...
                recommendations.append('Revisar y gestionar pagos de costos vencidos para evitar penalizaciones')
            ...
            if payment_rate < 80:
                ...
                recommendations.append('Implementar procesos de seguimiento de pagos más eficientes')
            ...
            if pending_amount and '0' not in pending_amount:
                ...
                recommendations.append('Planificar flujo de caja para cubrir costos pendientes')
            ...
            print(f"Error getting cost management insights: {e}")
            return {'alerts': [], 'recommendations': [], 'cost_kpis': {}}

    def _analyze_profitability_by_projects(self, df_edp: pd.DataFrame, df_costs: pd.DataFrame, costs_available: bool) -> List[Dict[str, Any]]:
        """Analyze profitability by projects using real cost data."""
        try:
            if 'proyecto' not in df_edp.columns:
                print("Warning: 'proyecto' column not found in EDP data")
                return []
            ...
            for proyecto, group in df_edp.groupby('proyecto'):
                ...
                if costs_available and not df_costs.empty:
                    ...
                    if 'proyecto' in df_costs.columns:
                        ...
                        if not project_cost_data.empty and 'proveedor' in project_cost_data.columns:
                            cost_breakdown = project_cost_data.groupby('proveedor')['importe_neto'].sum().to_dict()
                    else:
                ...
                projects_analysis.append({
                    ...
                    'revenue_share': round((total_revenue / df_edp['monto_aprobado'].sum() * 100) if df_edp['monto_aprobado'].sum() > 0 else 0, 1)
                })
            ...
            print(f"Error in _analyze_profitability_by_projects: {e}")
            return []
    
    def _analyze_profitability_by_clients(self, df_edp: pd.DataFrame, df_costs: pd.DataFrame, costs_available: bool) -> List[Dict[str, Any]]:
        """Analyze profitability by clients using real cost data."""
        try:
            if 'cliente' not in df_edp.columns:
                print("Warning: 'cliente' column not found in EDP data")
                return []
            ...
            for cliente, group in df_edp.groupby('cliente'):
                ...
                try:
                    if 'fecha_aprobacion' in group.columns and 'fecha_pago' in group.columns:
                        paid_edps = group[group['estado'] == 'pagado']
                        if not paid_edps.empty:
                            fecha_aprobacion = pd.to_datetime(paid_edps['fecha_aprobacion'], errors='coerce')
                            fecha_pago = pd.to_datetime(paid_edps['fecha_pago'], errors='coerce')
                            valid_mask = pd.notna(fecha_aprobacion) & pd.notna(fecha_pago)
                            if valid_mask.any():
            ...
            print(f"Error in _analyze_profitability_by_clients: {e}")
            return []
    
    def _analyze_profitability_by_managers(self, df_edp: pd.DataFrame, df_costs: pd.DataFrame, costs_available: bool) -> List[Dict[str, Any]]:
        """Analyze profitability by managers using real cost data."""
        try:
            if 'jefe_proyecto' not in df_edp.columns:
                print("Warning: 'gestor' column not found in EDP data")
                return []
            ...
            for gestor, group in df_edp.groupby('jefe_proyecto'):
                ...
                approved_count = len(group[group['estado'].isin(['pagado', 'validado'])])
                approval_rate = (approved_count / edp_count * 100) if edp_count > 0 else 0
                ...
                unique_clients = group['cliente'].nunique() if 'cliente' in group.columns else 0
                unique_projects = group['proyecto'].nunique() if 'proyecto' in group.columns else 0

...
class CostRepository(BaseRepository):
    ...
    def _model_to_row_values(
        ...
        field_mapping = {
            ...
            "fecha_factura": (
                cost.fecha_factura.isoformat() if cost.fecha_factura else ""
            ),
            "fecha_recepcion": (
                cost.fecha_recepcion.isoformat() if cost.fecha_recepcion else ""
            ),
            "fecha_vencimiento": (
                cost.fecha_vencimiento.isoformat() if cost.fecha_vencimiento else ""
            ),
    ...
    def get_costs_summary(self, project_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error getting costs summary: {e}")
            return {
                "total_costs": 0,
                "paid_costs": 0,
                "pending_costs": 0,
